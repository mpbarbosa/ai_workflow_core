...
git fetch --tags
git tag -l
# Output: v1.0.0, v1.0.1, v1.1.0, etc.
```
...
cd ..
git add .workflow_core
git commit -m "Update ai_workflow_core from v1.0.0 to v1.1.0"
...
cd .workflow_core
git fetch --tags
git checkout v1.0.0  # Known-good version
cd ..
git add .workflow_core
git commit -m "EMERGENCY: Rollback ai_workflow_core to v1.0.0"
git push
...
- **ai_workflow_core**: v1.0.0 (pinned)
...
- Issues: None
- Rollback plan: Revert to v1.0.0 if issues arise
...
- ai_workflow_core: >= v1.0.0, < v2.0.0 (no breaking changes)
...
strategy:
  matrix:
    workflow_core_version: ['v1.0.0', 'v1.1.0', 'main']
```

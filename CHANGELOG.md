...
- **`README.md`** — Updated version badge to 1.2.1, added TypeScript loader section, added
...
### `prompt_roles.yaml` *(new in v1.2.1)*
...
As of v1.2.1, each persona uses `role_ref: <key>` to reference its role definition
...
Prior to v1.2.1, each persona in `ai_helpers.yaml` carried its own inline `role_prefix:`
...
After the v1.2.1 refactor, persona entries in `ai_helpers.yaml` use `role_ref:` instead

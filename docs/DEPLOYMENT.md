...
git submodule add -b v1.0.2 https://github.com/mpbarbosa/ai_workflow_core.git .workflow_core
...
git commit -m "chore: Add ai_workflow_core v1.0.2 submodule"
```
...
git tag -a deploy/ai-workflow-v1.0.2 -m "Successfully deployed ai_workflow_core v1.0.2"
git push origin deploy/ai-workflow-v1.0.2
```
...
# 4. Review changes
git log --oneline $(git describe --tags --abbrev=0)..v1.2.7
...
# 7. Commit update
git add .workflow_core
git commit -m "chore: Update ai_workflow_core to v1.2.7"
```
...
./scripts/update-workflow-core.sh v1.2.7
```
...
# Example: Rolling back from v1.2.7 to v1.0.2
git checkout v1.0.2
...
# 3. Commit rollback
git add .workflow_core
git commit -m "fix: Rollback ai_workflow_core to v1.0.2 due to [issue]"
...
# Check for tampering
cd .workflow_core
git verify-tag v1.0.2  # If tags are signed
```
...
```bash
cd .workflow_core
git checkout v1.0.2  # or desired version
cd ..
git add .workflow_core
git commit -m "fix: Lock workflow_core to v1.0.2"
```
...
for env in dev staging prod; do
  ssh $env "cd /path/to/project && \
    ...
    cd .. && \
    git add .workflow_core && \
    git commit -m 'chore: Standardize workflow_core to v1.0.2'"
done
```

...
git diff v1.0.0..v2.0.0 -- config/.workflow-config.yaml.template
cd ..
...
git commit -m "Upgrade ai_workflow_core from v1.0.0 to v2.0.0"
```
...
cd .workflow_core
git log --oneline v1.0.0..HEAD -- config/.workflow-config.yaml.template
cd ..
```
...
# Add to README.md:
echo "Required ai_workflow_core version: v1.0.0" >> README.md

...
```bash
# Regenerate config/ai_helpers.yaml from sub-files
python3 scripts/build_ai_helpers.py --validate
# or:
npm run build:helpers

# Validate all sub-files and the merged artifact against the JSON schema
python3 scripts/validate_ai_helpers.py
# or:
...
`config/ai_helpers.schema.json` defines the `PersonaConfig` shape (mirroring the TypeScript interface in `olinda_prompter/.workflow_core/src/types.ts`). The validator uses this schema to check every persona in every sub-file.

### Backward Compatibility

Consumer projects (`ai_workflow.js`, `olinda_prompter`) load `config/ai_helpers.yaml` via hardcoded paths and are not affected by the sub-file split. The generated artifact keeps those integrations working without any changes.

---

## File Structure (Generated Artifact)

The generated artifact (`config/ai_helpers.yaml`) is organized as follows:

```yaml
# WARNING: GENERATED FILE — do not edit directly.
# Edit sub-files in config/ai_helpers/ and regenerate.

# 1. REUSABLE BEHAVIORAL GUIDELINES
#    - YAML anchors: &behavioral_actionable, &behavioral_structured, &behavioral_generative
#    - Used by personas throughout the file via aliases (*behavioral_*)

# 2. DOMAIN PROMPT GROUPS (61 total personas)
#    - documentation_prompts  (3 personas: doc_analysis, consistency, technical_writer)
#    - frontend_ux_prompts    (4 personas: front_end, e2e, ui_ux, tui)
#    - engineering_prompts    (5 personas: requirements, test_strategy, etc.)
#    - workflow_steps         (12 personas: step2…step14, resilience, lint, config)
#    - language_tables        (3 lookup tables: documentation, quality, testing)
#    - specialist_prompts     (22 personas: AWS, JS, TS, debuggers, etc.)
#    - library_prompts        (8 personas: library/framework engineering)
#    - review_prompts         (7 personas: security, accessibility, API, CLI, etc.)
```

**Total Size**: ~6,077 lines (v6.8.29)

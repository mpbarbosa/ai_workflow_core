...
wget https://github.com/mpbarbosa/ai_workflow_core/archive/refs/tags/v1.0.2.tar.gz
tar -xzf v1.0.2.tar.gz
cp -r ai_workflow_core-1.0.2/config/* .
```
...
cd .workflow_core
git fetch --tags
git checkout main  # or latest tag like v1.0.2
cd ..
...
cd .workflow_core
git fetch --tags
git checkout v1.0.2  # Pin to v1.0.2
cd ..
git add .workflow_core
...
```bash
cd .workflow_core
git checkout v1.2.7  # Update to v1.2.7
cd ..
git add .workflow_core
...
**Major versions** (e.g., v1.0.2 → v2.0.0):
- Breaking changes
- Migration guide required
- Significant new features

**Minor versions** (e.g., v1.0.2 → v1.2.7):
- New features
- Backward compatible
- Safe to update

**Patch versions** (e.g., v1.0.2 → v1.0.2):
- Bug fixes only
- No breaking changes
...
cd ..
git add .workflow_core
git commit -m "chore: rollback ai_workflow_core to v1.0.2"
```

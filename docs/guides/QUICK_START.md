...
project:
  ...
  version: "{{VERSION}}"                # → "1.0.0" (no 'v' prefix)
...
git commit -m "chore: integrate ai_workflow_core v1.0.0"
```
...
# Should show: <commit-hash> .workflow_core (v1.0.0 or similar)
```
...
- `{{VERSION}}` → Use "1.0.0" format (no 'v' prefix)
